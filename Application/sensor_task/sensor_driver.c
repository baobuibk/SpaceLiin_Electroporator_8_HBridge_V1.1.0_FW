...
static uint16_t Sensor_Calib_Timeout = LSM6DSOX_CALIB_TIMEOUT;
...
static bool LSM6DSOX_is_value_ready(Sensor_Read_typedef read_type,
		uint8_t *p_LSM6DSOX_RX_buffer);
static void LSM6DSOX_read_uncompensated_value(Sensor_Read_typedef read_type,
		uint8_t *p_LSM6DSOX_RX_buffer);
...
static bool BMP390_is_value_ready(Sensor_Read_typedef read_type,
		uint8_t *p_status_value);
static void BMP390_read_uncompensated_value(Sensor_Read_typedef read_type,
		uint8_t *p_BMP390_RX_buffer);
static void BMP390_compensate_value(Sensor_Read_typedef read_type,
		uint8_t *p_BMP390_RX_buffer);
...
/* :::::::::: I2C Command :::::::: */
static void I2C_Mem_Write_IT(I2C_TypeDef *I2Cx, uint8_t devAddress,
		uint8_t memAddress, uint8_t *data, uint8_t size);
static void I2C_Mem_Read_IT(I2C_TypeDef *I2Cx, uint8_t devAddress,
		uint8_t memAddress, uint8_t *data, uint8_t size);
...
static bool LSM6DSOX_init(void) {
	...
	case 2: {
		...
		Sensor_RX_buffer[0] = 0x00;		//0x02 for use user offset
		I2C_Mem_Write_IT(I2C_HANDLE, LSM6DSOX_ADDR, 0x16, Sensor_RX_buffer, 1);
		Sensor_Read_State = 3;
		return 0;
	}
	...
	case 3: {
		...
		Sensor_RX_buffer[0] = 0x09;
		I2C_Mem_Write_IT(I2C_HANDLE, LSM6DSOX_ADDR, 0x17, Sensor_RX_buffer, 1);
		Sensor_Read_State = 4;
		return 0;
	}
...
static bool LSM6DSOX_read_value(Sensor_Read_typedef read_type) {
	...
	case SENSOR_CHECK_STATUS_STATE: {
		I2C_Mem_Read_IT(I2C_HANDLE, LSM6DSOX_ADDR, LSM6DSOX_STATUS_REG_ADDR,
				Sensor_RX_buffer, 1);
...
static void LSM6DSOX_read_uncompensated_value(Sensor_Read_typedef read_type,
	...
	switch (read_type) {
	case SENSOR_READ_GYRO:
		I2C_Mem_Read_IT(I2C_HANDLE, LSM6DSOX_ADDR, LSM6DSOX_GYRO_REG_ADDR_BASE,
				p_LSM6DSOX_RX_buffer, LSM6DSOX_GYRO_REG_SIZE);
		break;

	case SENSOR_READ_ACCEL:
		I2C_Mem_Read_IT(I2C_HANDLE, LSM6DSOX_ADDR, LSM6DSOX_ACCEL_REG_ADDR_BASE,
				p_LSM6DSOX_RX_buffer, LSM6DSOX_ACCEL_REG_SIZE);
		break;

	case SENSOR_READ_LSM6DSOX:
		I2C_Mem_Read_IT(I2C_HANDLE, LSM6DSOX_ADDR, LSM6DSOX_READ_ALL_ADDR_BASE,
				p_LSM6DSOX_RX_buffer, LSM6DSOX_READ_ALL_SIZE);
		break;
...
//	for (uint8_t i = 0; i < 6; i += 2)
//	{
//		p_LSM6DSOX_data[i] = (int16_t)((p_LSM6DSOX_RX_buffer[i + 1] << 8) | p_LSM6DSOX_RX_buffer[i]) * Accel_Calib_Factor;
//	}
...
static bool BMP390_init(void) {
	...
	case 4: {
		...
		BMP390_data.uncomp_data.NVM_PAR_T1 = (Sensor_RX_buffer[1] << 8)
				| Sensor_RX_buffer[0];
		BMP390_data.uncomp_data.NVM_PAR_T2 = (Sensor_RX_buffer[3] << 8)
				| Sensor_RX_buffer[2];
		BMP390_data.uncomp_data.NVM_PAR_T3 = Sensor_RX_buffer[4];
		BMP390_data.uncomp_data.NVM_PAR_P1 = (Sensor_RX_buffer[6] << 8)
				| Sensor_RX_buffer[5];
		BMP390_data.uncomp_data.NVM_PAR_P2 = (Sensor_RX_buffer[8] << 8)
				| Sensor_RX_buffer[7];
		BMP390_data.uncomp_data.NVM_PAR_P3 = Sensor_RX_buffer[9];
		BMP390_data.uncomp_data.NVM_PAR_P4 = Sensor_RX_buffer[10];
		BMP390_data.uncomp_data.NVM_PAR_P5 = (Sensor_RX_buffer[12] << 8)
				| Sensor_RX_buffer[11];
		BMP390_data.uncomp_data.NVM_PAR_P6 = (Sensor_RX_buffer[14] << 8)
				| Sensor_RX_buffer[13];
		BMP390_data.uncomp_data.NVM_PAR_P7 = Sensor_RX_buffer[15];
		BMP390_data.uncomp_data.NVM_PAR_P8 = Sensor_RX_buffer[16];
		BMP390_data.uncomp_data.NVM_PAR_P9 = (Sensor_RX_buffer[18] << 8)
				| Sensor_RX_buffer[17];
	...
	case 7: {
		...
		BMP390_read_uncompensated_value(SENSOR_READ_BMP390,
				&Sensor_RX_buffer[1]);
		Sensor_Read_State = 8;
		return 0;
	}
...
static bool BMP390_read_value(Sensor_Read_typedef read_type) {
	...
	case 2: {
		...
		I2C_Mem_Read_IT(I2C_HANDLE, BMP390_ADDR, BMP390_STATUS_REG_ADDR,
				Sensor_RX_buffer, 1);
...
static void BMP390_read_uncompensated_value(Sensor_Read_typedef read_type,
	...
	if (read_type == SENSOR_READ_TEMP) {
		I2C_Mem_Read_IT(I2C_HANDLE, BMP390_ADDR, BMP390_TEMP_REG_ADDR_BASE,
				p_BMP390_RX_buffer, BMP390_TEMP_REG_SIZE);
		return;
	}

	I2C_Mem_Read_IT(I2C_HANDLE, BMP390_ADDR, BMP390_READ_ALL_ADDR_BASE,
...
static void BMP390_compensate_altitude(void) {
	...
	Sensor_Altitude = (29.32914804) * (Sensor_Temp)
			* log(Sensor_Pressure / 101325.0);
}
...
//	return 1;
	float Calib;
	switch (Sensor_Calib_State) {
	case 0:				//begin calib
		Sensor_Calib_Count = 0;
		is_sensor_read_finished = false;
		Sensor_Read_Value(SENSOR_READ_ACCEL);
		Sensor_Calib_State = 1;
		Sensor_Calib_Timeout = LSM6DSOX_CALIB_TIMEOUT;
		Sensor_Calib_Data.x = 0;
		Sensor_Calib_Data.y = 0;
		Sensor_Calib_Data.z = 0;
		return 0;
		break;
	...
	case 1:
		if (is_sensor_read_finished) {
			...
			if (Sensor_Calib_Count > 19) {
				Sensor_Calib_State = 2;
				return 0;
			}
			Sensor_Calib_Data.x += Sensor_Accel.x;
			Sensor_Calib_Data.y += Sensor_Accel.y;
			Sensor_Calib_Data.z += Sensor_Accel.z;
			is_sensor_read_finished = false;
			Sensor_Read_Value(SENSOR_READ_ACCEL);
			return 0;
		} else {
			if (Sensor_Calib_Timeout > 0)
				Sensor_Calib_Timeout--;
			else
				Sensor_Calib_State = 5;		//Timeout
			return 0;
		}

		break;
	case 2:
		Sensor_Calib_Data.x /= 20;
		Sensor_Calib_Data.y /= 20;
		Sensor_Calib_Data.z /= 20;

		if((abs(Sensor_Calib_Data.x) > 100) || (abs(Sensor_Calib_Data.y) > 100)){
			UART_Printf(&RS232_UART, "\nLOG: Ensure the accelerometer is placed flat on the ground\n");
			Sensor_Calib_State = 0;
			return 0;
		}

		if(abs(Sensor_Calib_Data.z - 1000) < 100)	{
			Sensor_Calib_State = 0;
			UART_Printf(&RS232_UART, "Calib done\n\r");
					UART_Printf(&RS232_UART, "Scale Factor changes to %f\n\r", Accel_Calib_Factor);
			return 1;
		}
		Calib = Sensor_Calib_Data.z;

		Calib /= Accel_Calib_Factor;
		Calib = 1000 / Calib;
		Accel_Calib_Factor = Calib;
		Sensor_Calib_State = 0;
		return 0;
		break;

	case 5:
		UART_Printf(&RS232_UART, "\nInit fail, being reset...\n");
		Sensor_Calib_State = 0;
		return 0;
		break;
	}
